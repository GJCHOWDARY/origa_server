    ...
    try {
      ...
      m= date.getMonth()+1,d=date.getDate();
       ...
       //TODO: mongodump --authenticationDatabase admin --username admin --password CHOWDARY19 -d SmartHRM_Production -h localhost
        if (shell.exec(`mongodump --authenticationDatabase admin --username admin --password CHOWDARY19 -d ${config.database} -h localhost`).code !== 0) {
            shell.exit(1);
            }
            else{
                ...
                const msg = {
                    ...
                    html: `Hello...  <br /> <br /> Mongo Backup Completed for Exasol! `,
                };
              sgMail.send(msg); 
            }
      } catch (err) {
